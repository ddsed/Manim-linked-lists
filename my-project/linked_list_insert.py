...
class LinkedListScene(Scene):
    def construct(self):
        ...
        # Get input from user **before animation starts**
        node_values = list(map(int, input("Enter node values separated by space: ").split()))
        insert_num1, insert_num2 = map(int, input("Enter the two node values where the new node should be inserted: ").split())
        new_value = int(input("Enter the new node value: "))

        # Create nodes
        nodes = [LinkedListNode(value) for value in node_values]
        ...
        # Insert a new node
        self.insert_node_between(nodes, insert_num1, insert_num2, new_value)

    def insert_node_between(self, nodes, num1, num2, new_value):
        # Find the reference nodes for insertion + color code them
        node1 = next((node for node in nodes if node.text.text == str(num1)), None)
        node2 = next((node for node in nodes if node.text.text == str(num2)), None)
        ...
        self.play(
            node1.box.animate.set_fill(GREEN, opacity=0.45),
            node2.box.animate.set_fill(GREEN, opacity=0.45),
            FadeIn(textfunc)
        )
        ...
        # Arrows visibility
        node1.next_arrow.set_opacity(1)
        new_node.next_arrow.set_opacity(1)
        ...
        # After the animation, remove updaters to stop updates
